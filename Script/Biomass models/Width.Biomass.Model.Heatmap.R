...
# List of species
species_list <- c("Actaea", "Adiantum", "Allium", "Caulophyllum",
                  "Polygonatum", "Quercus","Sanguinaria","Tiarella","Trillium")
...
for (species in species_list) {
  ...
  tryCatch({
    ...
    full_model_formula <- paste0(width_col, " ~ Year * deer + Year * Drybiomass.mean + deer * Drybiomass.mean + I.", species, " + (1 | site/Row)")
    full_model <- lmer(as.formula(full_model_formula), data = d4)
    
    reduced_models_formulas <- c(
      paste0(width_col, " ~ Year + deer + Drybiomass.mean + I.", species, " + (1 | site/Row)"),
      paste0(width_col, " ~ Year * deer + Drybiomass.mean + I.", species, " + (1 | site/Row)"),
      paste0(width_col, " ~ Year * Drybiomass.mean + deer + I.", species, " + (1 | site/Row)"),
      paste0(width_col, " ~ deer * Drybiomass.mean + Year + I.", species, " + (1 | site/Row)")
    )
...
# After the model fitting loop
for (species in species_list) {
  model_name <- paste0(species, "_best_width_model")
  
  if (!exists(model_name)) {
    cat("Model does not exist for", species, "\n")
  } else {
    cat("Model exists for", species, "\n")
    # Optional: Print a summary or check convergence

...
species_list <- c("Actaea", "Adiantum", "Geum", "Agrimonia", "Allium", "Arisaema", 
                  "Brachyelytrum", "Carex", "Caulophyllum", "Dryopteris", "Geranium", 
                  "Maianthemum", "Polygonatum", "Polygonum", "Polystichum", "Quercus", 
...
Trillium_best_model <- glmer(Trillium.final~ Year.order + worms * deer + (1 | site),family=binomial,data=d4)

# Loop through each species
for (species in species_list) {
  final_col <- paste0(species, ".final")
  ...
  tryCatch({
    # Model formula with three-way interaction
    full_model_formula <- paste0(species, ".final ~ Year.order * deer * worms + I.", species, " + (1 | site)")
    ...
    reduced_models_formulas <- c(
      paste0(species, ".final ~ Year.order + deer + worms + I.", species, " + (1 | site)"),
      paste0(species, ".final ~ Year.order * deer + Year.order + worms + I.", species, " + (1 | site)"),
      paste0(species, ".final ~ Year.order * worms + deer + worms + I.", species, " + (1 | site)"),
      paste0(species, ".final ~ deer * worms + Year.order + deer + I.", species, " + (1 | site)")
      # Add other reduced models as needed
    )
...
group1 <- c("Actaea", "Allium", "Caulophyllum", "Polygonatum")  # y-axis numbers only
group2 <- c("Sanguinaria")  # y-axis and x-axis (years) numbers
group3 <- c("Thalictrum", "Tiarella", "Trillium")  # x-axis (years) numbers only
special_strip_group <- c("") # Species with special strip background "Actaea", "Adiantum", "Geum", "Agrimonia"
...
multiplot <- grid.arrange(grobs = plot_list, ncol = 4) # Adjust the number of columns as needed

# Save as a high-quality TIFF image
ggsave("/Users/annisedobson/Dropbox (YSE)/Data/Lumb x deer/DeerxEW/Final figures/Survival.tiff", 
       plot = multiplot, 
       width = 10, 
       height = 8, 
       dpi = 300, 
